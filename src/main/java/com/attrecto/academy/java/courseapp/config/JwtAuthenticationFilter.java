...
public class JwtAuthenticationFilter  extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
	      ...
	      final String authorizationHeaderValue = request.getHeader(HttpHeaders.AUTHORIZATION);
	        ...
	        if (authorizationHeaderValue != null && StringUtils.startsWithIgnoreCase(authorizationHeaderValue, "Bearer ")) {
	            ...
	            try {
	                ...
	                SecurityContextHolder.getContext().setAuthentication(new JwtAuthentication(subject, 
	                		Collections.singletonList(map(maybeUser.get().getRole().name()))));
